...
$router->group(['prefix' => 'admin' , 'namespace' => 'Admin','middleware'=> 'cors'], function () use ($router) {
    ...
    $router->get('refuntOrdersoujiExceil', 'OrderController@refuntOrdersoujiExceil');//退费管理-退费搜集导出
    ...
    $router->post('ylnotify', 'NotifyController@ylnotify');//银联 支付回调 （lys）
    $router->post('yltest', 'OrderController@yltest');//银联测试支付（lys）
...
$router->group(['prefix' => 'admin' , 'namespace' => 'Admin','middleware'=> ['jwt.auth', 'cors','api'] ], function () use ($router) {
    ...
    $router->group(['prefix' => 'order'], function () use ($router) {
        ...
        $router->post('financeOrder', 'OrderController@financeOrder');//财务审核退费订单状态
        ...
        $router->post('addremark', 'OrderController@addremark');//添加备注
        //核对订单
