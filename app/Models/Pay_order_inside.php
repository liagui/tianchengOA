    ...
    public static function getBranchSchoolIncomeeLists($body=[],$timearr=[]){
        ...
        foreach ($list as $listk => &$listv){
           ...
           //查询分校名
            ...
            //成本
            $chengben = self::where($where)->whereBetween('create_time', [$school_start_time, $school_end_time])->sum('sing_Price');
            ...
            //收入 = 到账金额-报名费-退费金额-分校返佣-保证金
            $shouru = $actual_receipt - $chengben - $refundorderPrice - $baozhengjin - $lists['countPrice'];
