    ...
    public static function notarizeOrder($data){
        ...
        if($data['confirm_status'] == 1){
            ...
            if($order['first_pay'] == 3 || $order['first_pay']==4){
                $orderone = self::where(['name'=>$order['name'],'mobile'=>$order['mobile'],'course_id'=>$order['course_id'],'project_id'=>$order['project_id'],'subject_id'=>$order['subject_id'],'first_pay'=>1,'confirm_status'=>2,'fee_id'=>$order['fee_id']])->first();
                if(!empty($orderone)){
...
//                }
    ...
    public static function getBranchSchoolIncomeeLists($body=[],$timearr=[]){
        ...
        if(isset($body['school_id'])){
            ...
            $refundwhere['school_id'] = $body['school_id'];
        }
        $where['pay_status'] = 1;
        $where['del_flag'] = 0;
        //科目id&学科id
        ...
        foreach ($list as $listk => &$listv){
            ...
            $lists['intoreturn'] = sprintf("%.2f",$lists['intoreturn'] + $singorderPrice);
            //成本
            ...
            if ($schoolOne['level'] == 1) {
                ...
                $zhanshireturnschoolprice = $returnschoolprice + $returnschoolsingprice;
                ...
                if(!empty($seond_school_id)){
                    ...
                    foreach($seond_school_id as $onek=>$onev){
                        //到账
                        $oneprices = self::where(['school_id'=>$onev['id'],'pay_status'=>1,'confirm_status'=>2])->whereBetween('comfirm_time', [$school_start_time, $school_end_time])->sum('pay_price');
                        //扣税=到账金额*扣税比例
                        $tax_deductions = sprintf("%.2f", $oneprices * ($onev['tax_point'] / 100));
                        //税后金额=到账金额-扣税
                        $after_tax_amounts = $oneprices - $tax_deductions;
                        //成本
                        $sum_costs = self::where(['school_id'=>$onev['id'],'pay_status'=>1,'confirm_status'=>2])->whereBetween('comfirm_time', [$school_start_time, $school_end_time])->sum('sign_Price');
                        //实际到款=税后金额-成本
...
//                            $threereturnschoolprice = $threereturnschoolsingprice + $threereturnschoolprice;
                ...
                if(!empty($three_school_id)){
                    foreach($three_school_id as $onek=>$onev){
                        //到账
                        $threeprice = self::where(['school_id'=>$onev['id'],'pay_status'=>1,'confirm_status'=>2])->whereBetween('comfirm_time', [$school_start_time, $school_end_time])->sum('pay_price');
                        //扣税=到账金额*扣税比例
...
//                //查看三个级别分校是否都在里面
                ...
                foreach ($list as $onlek=>$onlev){
                    ...
                    if($onlev['level'] == 3){
                        if(!in_array($onlev['parent_id'],$onleschool)){
                            $erjischool = School::where(['id'=>$onlev['parent_id'],'is_open'=>0,'is_del'=>0])->first();
                            array_push($xinschoolarr,$erjischool['id']);
                            if(!in_array($erjischool['parent_id'],$onleschool)){
                                $yijischool = School::where(['id'=>$erjischool['parent_id'],'is_open'=>0,'is_del'=>0])->first();
                                array_push($xinschoolarr,$yijischool['id']);
                            }
                        }
                    }
                ...
                if(!empty($xinschoolarr)){
                    foreach ($xinschoolarr as $xinschoolarrk=>$xinschoolarrv){
                            $xinschoolIdarr = DB::table('school')->selectRaw('
                               any_value(school.id) as school_id ,
                               any_value(count(school.id)) as t_count ,
                               any_value(school.one_extraction_ratio) as one_extraction_ratio ,
                               any_value(school.two_extraction_ratio) as two_extraction_ratio ,
                               ...
                               any_value(0) as major_id,
                               any_value(0) as sign_Price'
                                )->where('id',$xinschoolarrv)->get()->toArray();
                            $list = array_merge($list,$xinschoolIdarr);
                        }
                }
                //循环获取相关信息
                foreach ($list as $k => $v) {
...
//                }
                ...
                return ['code' => 200, 'msg' => '获取列表成功', 'data' => ['list' => $newarr]];
//            }
        }
