        ...
        if($count > 0){
            ...
            foreach($list as $k=>$v){
                ...
                if(isset($body['school_id']) && !empty($body['school_id']) && $body['school_id'] > 0){
                    ...
                    //报名总费用
                    $enroll_price   = Pay_order_inside::where('school_id' , $body['school_id'])->where('create_time', '>=' , $startTime)->where('create_time', '<=' , $endTime)->sum('sign_Price');

                    //成本总费用 
                    $prime_cost     = Pay_order_inside::where('school_id' , $body['school_id'])->where('create_time', '>=' , $startTime)->where('create_time', '<=' , $endTime)->sum('sum_Price');

                    //实际佣金总费用
                    $actual_commission = Pay_order_inside::where('school_id' , $body['school_id'])->where('create_time', '>=' , $startTime)->where('create_time', '<=' , $endTime)->sum('actual_commission');
                } else {
                    $school_name  = "所有分校";
                    ...
                    //报名总费用
                    $enroll_price   = Pay_order_inside::where('create_time', '>=' , $startTime)->where('create_time', '<=' , $endTime)->sum('sign_Price');

                    //成本总费用 
                    $prime_cost     = Pay_order_inside::where('create_time', '>=' , $startTime)->where('create_time', '<=' , $endTime)->sum('sum_Price');

                    //实际佣金总费用
                    $actual_commission = Pay_order_inside::where('create_time', '>=' , $startTime)->where('create_time', '<=' , $endTime)->sum('actual_commission');
                }
                ...
                if(isset($body['category_id']) && !empty($body['category_id'])){
                    ...
                    if($subject_id && $subject_id > 0){
                        //学科名称
                        $subject_name = Project::where('parent_id' , $v['project_id'])->where('id' , $subject_id)->value('name');
                    } else {
                        $subject_name = "所有学科";
                    }
                } else {
